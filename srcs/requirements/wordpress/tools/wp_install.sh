...
if ! wp user get "$WORDPRESS_USER" --allow-root > /dev/null 2>&1; then
	...
	TITTLE='<div class="text-center"><img src="'$AVATAR_URL'" class="rounded-circle" alt="Avatar" /><br />Puerto Escondido, MX</div>'
	wp term create category Blog --allow-root
	BLOG=$(wp term list category --name=Blog --field=term_id --allow-root)
	POST_ID=$(wp post create --post_title="$TITTLE" --post_status=publish --post_content="$POST_CONTENT" --post_author="$USER_ID" --post_category="$BLOG" --allow-root --porcelain ) #--post_thumbnail="$ATTACHMENT_ID
